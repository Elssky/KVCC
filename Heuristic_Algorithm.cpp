...
TIntVIntV BkVCC::Seeding(PUNGraph G, int k, int alpha)
{
	...
	try
	{
		TFIn inFile("D:/Git Project/KVCC-ENUM//seedgraph/" + dataset + "_k=" + TStr(k_str) + "_alpha=" + TStr(alpha_str) + "_algorithm=" + TStr(alg_str) + ".seed");
		G_S.Load(inFile);
	}
	catch (TPt<TExcept>)
	{
	...
	TFOut outFile("D:/Git Project/KVCC-ENUM//seedgraph/" + dataset + "_k=" + TStr(k_str) + "_alpha=" + TStr(alpha_str) + "_algorithm=" + TStr(alg_str) + ".seed");
	G_S.Save(outFile);
	//cout << G_S.Len();
	...
	for (int i = 0; i < G_S.Len();) {
		for (int j = i + 1; j < G_S.Len();) {
			...
			if (IsMergeValid(GI1, GI2)) {
				cout << "Before Merging:" << GI1.Len() << " " << GI2.Len() << endl;
				/*cout << "GI1:[";
				for (TIntV::TIter TI = GI1.BegI(); TI < GI1.EndI(); TI++) {
					cout << *TI << ",";
				}
				...
				cout << "After Merging:" << G_Union.Len() << endl;
				/*cout << "G_Union:[";
				for (TIntV::TIter TI = G_Union.BegI(); TI < G_Union.EndI(); TI++) {
					cout << *TI << ",";
				}
